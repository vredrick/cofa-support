...
### Election district choices

`public/elections/districts.mjs` transcribes the National Election Office’s [electoral division list](https://www.fsmned.fm/PDF/new_electoral_divisions_%20by_state.pdf), checked against both pages of the supplied PDF on September 8, 2026. Both election forms, including prior registration details, show state-dependent place choices grouped by ED. The PDF receives only the selected ED number; village and municipality remain separate fields because the source mixes municipalities, islands and polling places. Changing state clears the dependent selection. Preserve the two separate Falalop entries in Yap ED 2 and ED 3. Yap has five divisions in this source; Kosrae has one district with six polling places.
